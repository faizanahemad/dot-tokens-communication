...
class SamplingMixin:
    def _sampling(self, logits, sampling_method, temperature, top_k=50, top_p=0.95):
        ...
        if sampling_method == "greedy":
            ...
            top_logprobs_dict = [{self.tokenizer.decode([idx.item()]): torch.log(prob).item() for idx, prob in zip(top_k_indices[i], top_k_probs[i])} for i in range(logits.size(0))]
            probs = torch.log(torch.gather(probs, 1, next_token.unsqueeze(-1)).squeeze(-1))
        elif sampling_method == "sample":
            probs = torch.nn.functional.softmax(logits / temperature, dim=-1)
            next_token = torch.multinomial(probs, num_samples=1).squeeze(1)
            probs = torch.gather(probs, 1, next_token.unsqueeze(-1)).squeeze(-1)
        ...
        return next_token, probs, top_logprobs_dict